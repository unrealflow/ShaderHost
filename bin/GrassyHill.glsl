...
void main()
{
    ...
    vec2 xy = TexCoords.xy / iResolution.xy;
	...
	// #ifdef STEREO
	// float isCyan = mod(TexCoords.x + mod(TexCoords.y,2.0),2.0);
	// #endif
	...
	camTar.y = cameraPos.y+iMouse.y*0.01;
	
	// float roll = .4*sin(gTime+.5);
	vec3 cw = normalize(camTar-cameraPos);
	// vec3 cp = vec3(sin(roll), cos(roll),0.0);
	vec3 cp = vec3(0.0, 1.0,0.0);
	vec3 cu = cross(cw,cp);//摄像机右
	vec3 cv = cross(cu,cw);//摄像机上
